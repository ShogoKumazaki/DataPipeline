...
- **フェーズ 1**: パイプライン本体の実装を優先し、まず「データが毎日スプレッドシートに反映される」状態を実現する（Cloud Scheduler → Cloud Run Jobs の直接起動、スケジュールクエリは手動設定）
- **フェーズ 2**: 稼働後に Cloud Workflows の導入や CI/CD パイプラインの構築など、運用基盤を整備する
